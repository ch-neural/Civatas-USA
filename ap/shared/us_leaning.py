...
def load_county_pvi(path: str | None = None) -> dict[str, float]:
    ...
    Search order:
      ...
      2. ``CIVATAS_USA_DATA_DIR`` env var → {dir}/elections/leaning_profile_us.json
      3. sibling ``us_data/elections/leaning_profile_us.json`` (docker layout)
      4. upward walk for ``data/elections/leaning_profile_us.json`` (dev layout)
    """
    import os
    global _county_pvi, _loaded_path
    if path is None:
        env_dir = os.environ.get("CIVATAS_USA_DATA_DIR")
        if env_dir:
            cand = Path(env_dir) / "elections" / "leaning_profile_us.json"
            if cand.exists():
                path = str(cand)
    if path is None:
        sibling = Path(__file__).resolve().parent / "us_data" / "elections" / "leaning_profile_us.json"
        if sibling.exists():
            path = str(sibling)
    if path is None:
