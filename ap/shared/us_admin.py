...
def load_fips_index(data_dir: str | os.PathLike | None = None) -> None:
    ...
    if base is None:
        ...
        if env_dir and Path(env_dir).is_dir():
            base = Path(env_dir)
    if base is None:
        # Sibling path: shared/us_data/ (works inside the docker bind mount,
        # where this file lives at /app/shared/us_admin.py).
        sibling = Path(__file__).resolve().parent / "us_data"
        if (sibling / "census").is_dir():
            base = sibling
    if base is None:
        # Best-effort upward walk: locates Civatas-USA/data/census/ during dev.
